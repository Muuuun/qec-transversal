...
- structural report fields from the underlying theory: self-duality,
  ...
  parts;
- a built-in registry of 59 named codes across twenty families
  (bivariate/coprime/trivariate bicycle, **self-dual** bivariate bicycle,
  generalized bicycle, hypergraph product, toric/surface, colour, La-cross,
  lifted product, Kasai quasi-cyclic, APM Kasai-template, Cornucopia,
  subset-inclusion, doubled, iceberg, Reed-Muller, grid); see
  `docs/qldpc-survey.md` and the published zoo for certified results.  These
  counts are generated into the zoo page from `zoo_data.json`, so treat the
  zoo as the source of truth if this line drifts;
- the self-dual bivariate bicycle codes of Liang-Chen (arXiv:2510.05211) as
  the registry's **positive LDPC control**: `H_X = H_Z = [F | F^T]` on a
  twisted torus `Z^2 / <a1, a2>`, weight-8 doubly-even checks, and a
  certified *strict* transversal H/S (logical group order 6 on all four
  instances) — so the census's qLDPC triviality results are never read as a
  claim about check sparsity;
...
- the **one-block generated group** (`oneblock.py`, `analyze_one_block`):
  the group generated by *all* depth-one layers of a single block together —
  strict gates, a layer over every certified matching, and monomial
  automorphism gates — checked against `|Sp(2k,2)|` and published as the
  zoo's `one-block group` column, with `single_matching_fullness` deciding
  whether one matching plus automorphisms already suffices.  Two scope
  limits, both deliberate: the Levi/CNOT units of each matching are **not**
  collected (so this is a different generating set from ``N_2fold`` above —
  neither group contains the other), and `is_full` is **one-sided**, True or
  None but never False, because involutions are sampled rather than
  enumerated; a non-full row is a lower bound, never a no-go;
...
- a signed code representation $`(H, \sigma)`$ (`signed.py`): any engine's
  symplectic output — strict, fold, partition, monomial — *can be* lifted to
  an exact Stim tableau and sign-verified through `verify_sign_exact`, with
  an explicit Pauli correction; the automated pipeline currently does this
  for the strict **diagonal** generators (`phase.py`).  Scope note: the
  published census orders (strict, fold, monomial, one-block) are symplectic
  actions **modulo Paulis and global phases**.  That is sound — the sign
  defect of a Clifford on the stabilizer group is a linear character, so a
  Pauli correction always exists and never changes the symplectic action —
  but the explicit circuit-level sign fix is not computed for fold,
  partition, monomial or one-block layers.  Do not read those orders as
  sign-verified circuit claims;
...
- **group words**: `verify_logical_gate` returns an explicit three-layer
  implementation for a requested target (that part is implemented, above),
  but not a decomposition of an arbitrary group element into named
  generators;
...
If this tool or the Transversal Gate Zoo is used in research, cite
...
@software{qiao2026qectransversal,
  ...
  note   = {Transversal Gate Zoo: \url{https://muuuun.github.io/qec-transversal/}}
}
```
