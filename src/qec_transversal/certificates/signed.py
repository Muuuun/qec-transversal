...
from ..utils.gf2 import left_kernel_basis as _left_kernel_basis
...
def _decompose_over(rows: np.ndarray, target: np.ndarray) -> np.ndarray | None:
    """Coefficients expressing ``target`` over the rows of ``rows`` (or None)."""

    return solve_gf2(np.asarray(rows, dtype=np.uint8).T, target)
