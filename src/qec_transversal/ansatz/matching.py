...
class MatchingGenerator:
    ...
    #: The dense ``2n x 2n`` physical symplectic matrix of the fold layer.
...
class FoldHadamard:
    logical_symplectic: BinaryMatrix
    certificate: dict[str, bool]
    #: The dense ``2n x 2n`` physical symplectic matrix: transversal H
    #: followed by the permutation, i.e. ``[[0, P], [P, 0]]``.
    matrix: BinaryMatrix | None = None
...
class MatchingAnalysis:
    ...
    def _generator(self, family: Family, parameter: BinaryMatrix) -> MatchingGenerator:
        ...
        sigma = sigma_matrix(parameter, self.pairs, code.n)
        identity_n = np.eye(code.n, dtype=np.uint8)
        zero_n = np.zeros((code.n, code.n), dtype=np.uint8)
        if family == "Z":
            physical = np.block([[identity_n, sigma], [zero_n, identity_n]])
        else:
            physical = np.block([[identity_n, zero_n], [sigma, identity_n]])
        return MatchingGenerator(
            family=family,
            parameter=parameter.copy(),
            logical_symplectic=formula,
            certificate=certificate,
            matrix=physical.astype(np.uint8),
        )
    ...
    def _fold_hadamard(self) -> FoldHadamard:
        ...
        zero_n = np.zeros((n, n), dtype=np.uint8)
        physical = np.block([[zero_n, self.perm], [self.perm, zero_n]]).astype(np.uint8)
        return FoldHadamard(
            logical_symplectic=logical, certificate=certificate, matrix=physical
        )
